...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int checkNum = 1; checkNum != last; checkNum++) {
            boolean isInSol = false;
        ...
        for (int x = 0; x < solution.length; x++) {
            for (int y = 0; y < solution[x].length; y++) {
                int aD = arrowDirection(x, y);
                if (solution[x][y] == 1) {
                    _board[x][y] = new Sq(x, y, 1, true, aD, 0);
                } else if (solution[x][y] == last) {
                    _board[x][y] = new Sq(x, y, last, true, 0, 0);
                } else {
                    _board[x][y] = new Sq(x, y, 0, false, aD, -1);
                }
            }
        }
        ...
        for (int x = 0; x < solution.length; x++) {
            for (int y = 0; y < solution[x].length; y++) {
                _solnNumToPlace[solution[x][y]] = get(x, y).pl;
            }
        }
        for (Sq[] col : _board) {
            for (Sq sq : col) {
        ...
        for (Sq s : _allSquares) {
            s._successors = allSuccessors(s.pl, arrowDirection(s.pl.x, s.pl.y));
            for (Place p : s.successors()) {
                get(p.x, p.y)._predecessors.add(s.pl);
            }
        }
    }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _solution.length; x++) {
            for (int y = 0; y < _solution[x].length; y++) {
                int aD = arrowDirection(x, y);
                if (_solution[x][y] == 1) {
                    _board[x][y] = new Sq(x, y, 1, true, aD, 0);
        ...
        for (Sq s : _allSquares) {
            s._successors = allSuccessors(s.pl, arrowDirection(s.pl.x, s.pl.y));
            for (Place p : s.successors()) {
                get(p.x, p.y)._predecessors.add(pl(s.x, s.y));
            }
        }
        ...
        for (Sq s : _allSquares) {
            if (s.group() == 0 && s.successor() == null) {
                for (Place p : allSuccessors(s.x, s.y, s._dir)) {
                    Sq pSquare = get(p.x, p.y);
                    if (pSquare.sequenceNum() == s.sequenceNum() + 1) {
                        s.connect(pSquare);
                        hasChanged = true;
                    }
                }
    ...
    private int arrowDirection(int x, int y) {
        ...
        for (int i = 0; i < _solution.length; i++) {
            for (int j = 0; j < _solution[x].length; j++) {
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            int thisOldNum = sequenceNum();
            int s1OldNum = s1.sequenceNum();

            if ((sequenceNum() != 0)) {
                Sq ptr = this;
                for (int i = _sequenceNum; ptr != null; i++) {
            ...
            if (s1.sequenceNum() != 0) {
                ...
                Sq ptr = s1;
                for (int i = s1._sequenceNum; ptr != null; i--) {
                    if (ptr.sequenceNum() == 0) {
                        ptr._sequenceNum = i;
                    }
            ...
            Sq pointer = this.successor();
            for (; pointer != null; pointer = pointer.successor()) {
                pointer._head = _head;
            }
            if ((thisOldNum == 0 && sequenceNum() != 0)) {
                releaseGroup(tGroup);
            } else if (s1OldNum == 0 && s1.sequenceNum() != 0) {
                releaseGroup(sgroup);
            }
        ...
        /*** Checks if a Sq is in a one element group.
         * @param s is the Square that you want to check if it is a one
         * element group.
         * @return truth or false depending on the element
         * in part of a 1 element group.*/
        boolean checkFor1ElemG(Sq s) {
        ...
        /*** Checks whether a successor or
         *  predecessor has a fixed number.
         * @param way is the String that
         * determines whether to use successors or predecessorss
         * @param s the Sq (Square)
         *@return returns a boolean, determining whether
         * a prodecessor or a successor has a fixed number.*/
        boolean succHasFixedNum(Sq s, String way) {
            Sq pointer = s;
            if (way.equals("succ")) {
                for (; pointer != null; pointer = pointer.successor()) {
                    if (pointer._hasFixedNum) {
                        return true;
                    }
                }
                return false;
            ...
            if (way.equals("pred")) {
                for (; pointer != null; pointer = pointer.predecessor()) {
                    if (pointer._hasFixedNum) {
                        return true;
                    }
                }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (checkFor1ElemG(this) && checkFor1ElemG(next)) {

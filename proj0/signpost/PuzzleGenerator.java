...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        PlaceList possibleSucc = start.successors();
        if (possibleSucc.size() == 1) {
            return model.get(possibleSucc.get(0));
        }
        ArrayList<Sq> retSol = new ArrayList<>();
        int numOfSucc = 0;
        if (start.sequenceNum() != 0) {
            for (Place succ : possibleSucc) {
                Sq sq = model.get(succ);
                if (sq.sequenceNum() == start.sequenceNum() + 1
                    && start.connectable(sq)) {
                    return sq;
                }
            }
        }
        for (Place succPlace : possibleSucc) {
            Sq sq = model.get(succPlace);
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        PlaceList possiblePred = end.predecessors();
        ArrayList<Sq> connectableSqs = new ArrayList<>();
        for (Place p : possiblePred) {
            Sq sq = model.get(p);
            if (sq.connectable(end)) {
                connectableSqs.add(sq);
            }
        }
        if (connectableSqs.size() == 1) {
            return connectableSqs.get(0);
        } else {
            for (Sq sq : connectableSqs) {
                if (sq.connectable(end)
                    && sq.sequenceNum() == end.sequenceNum() - 1) {
                    return sq;
                }
            }
        }

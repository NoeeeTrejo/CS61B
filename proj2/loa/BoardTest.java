...
public class BoardTest {
    ...
    public void testCopyBoard() {
        Board b1 = new Board(BOARD1, BP);
        Board copy = new Board(BOARD2, BP);
        copy.copyFrom(b1);
        assertEquals("The boards are the same!", b1, copy);
    ...
    public void testMove1() {
        ...
        Board b1 = new Board(BOARD1, BP);
        Piece p12 = b1.get(sq(3, 4));
        Piece o12 = b1.get(sq(5, 2));
        b1.makeMove(mv("f3-d5"));
        assertEquals("square d5 after f3-d5", BP,
                b1.get(sq(3, 4)));
        assertEquals("square f3 after f3-d5", EMP,
                b1.get(sq(5, 2)));
        assertEquals("Check move count for board 1 after one move",
                     1, b1.movesMade());
        b1.retract();
        Piece p = b1.get(sq(3, 4));
        Piece o = b1.get(sq(5, 2));
        assertEquals("Square at (5, 2) was not restored",
                b0.get(sq(5, 2)), b1.get(sq(5, 2)));
        assertEquals("Square at (3, 4) was not restored",
                b0.get(sq(3, 4)), b1.get(sq(3, 4)));
        assertEquals("Check for board 1 "
                + "restored after retraction", b0, b1);

        assertEquals("Check move count for board "
                        + "1 after move + retraction",
                     0, b1.movesMade());

        b1.makeMove(mv("f3-h5"));
        b1.retract();
        assertEquals("Square at (5, 2) was not restored",
                b0.get(sq("f3")), b1.get(sq("f3")));
        assertEquals("Square at (3, 4) was not restored",
                b0.get(sq("h5")), b1.get(sq("h5")));

        b1.makeMove(mv("f3-h1"));
        b1.retract();
        assertEquals("Square at (5, 2) was not restored",
                b0.get(sq("f3")), b1.get(sq("f3")));
        assertEquals("Square at (3, 4) was not restored",
                b0.get(sq("h1")), b1.get(sq("h1")));

        b1.makeMove(mv("f3-b3"));
        b1.retract();
        assertEquals("Square at (5, 2) was not restored",
                b0.get(sq("f3")), b1.get(sq("f3")));
        assertEquals("Square at (3, 4) was not restored",
                b0.get(sq("b3")), b1.get(sq("b3")));
